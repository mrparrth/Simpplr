 ...
 */
function _toast_(message, title = '', timeout = 4) {
...
function _tryAction_(functionName, showError = false, title = "Script") {
  try {
    functionName();
  } catch (error) {
...
function _generateKeyValuePairFromSheet_(sheet, startRow = 1, keyCol = 1, valueCol = 2) {
...
function _showOnlySheet_(sheetToKeep) {
  let sheets = SpreadsheetApp.getActive().getSheets().filter(sheet => !sheet.isSheetHidden())
